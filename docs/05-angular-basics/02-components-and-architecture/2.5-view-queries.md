...
@Component({
  ...
  template: `
    <form
      (ngSubmit)="addTask()"
      class="bg-white p-6 rounded-2xl shadow-lg mb-8"
    >
      <div class="flex gap-4">
        ...
        />
        <button type="submit" [disabled]="!newTaskTitle.trim()" class="...">
          Add Task
        </button>
      </div>
    </form>
...
>
  ...
  <!-- Subtle focus hint -->
  <div
    class="mt-2 text-xs opacity-0 group-hover:opacity-100 transition-all text-blue-500"
  >
    👆 Click to add task
  </div>
</div>

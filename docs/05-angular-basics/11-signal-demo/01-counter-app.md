...
@Component({
  ...
  template: `
    ...
    >
      <!-- Persistence Status -->
      <div
        class="mt-6 p-4 bg-white/10 backdrop-blur-sm rounded-2xl text-center"
      >
        <div class="text-indigo-100 font-medium">
          💾 Persisted in localStorage
        </div>
        <div class="text-sm text-indigo-200 mt-1">
          Refreshes survive • Clears on Reset
        </div>
      </div>

      <!-- Counter Display -->
      <div
        class="text-center my-6 text-8xl font-mono font-black text-white tracking-tight"
      >
        {{ count() }}
      </div>
      ...
      <div class="p-4 bg-white/10 backdrop-blur-sm rounded-2xl">
        ...
        <div class="space-y-2 max-h-40 overflow-y-auto">
          @for (ev of events(); track ev.id) {
            <div
              class="flex justify-between items-center p-2 bg-purple-500 rounded-lg"
            >
            ...
            <div class="text-center text-indigo-300 text-sm py-4">
              No events yet
            </div>
          }
        </div>
